...
## DEMO ANALYSIS 
#sub_a <- file_a %>% filter(BirthYear == "1987" & (LastInitial == "s" | LastInitial == "x"))
#sub_b <- file_b %>% filter(BirthYear == "1987" & (LastInitial == "s" | LastInitial == "x"))
#row.names(sub_a) <- seq(1,nrow(sub_a), by = 1)
#row.names(sub_b) <- seq(1,nrow(sub_b))
# nrow(sub_a) - 548 records
# nrow(t) - 341 records

sub_a <- read.csv("hcc_sub_a.csv")
sub_b <- read.csv("hcc_sub_b.csv")
...
compare_files_demo <- compare.linkage(sub_a, sub_b,
                                      blockfld = c("BirthYear", "LastInitial"),
                                      strcmp = c("LastName", "FirstName", "MiddleName"),
                                      exclude = c("FirstIssueDate", "BirthYear", "LastInitial"),
                                      strcmpfun = jarowinkler)

comparison_vectors_demo <- compare_files_demo$pairs %>% select(id1, id2, LastName, FirstName, MiddleName)
...
par(mfrow=c(1,3))
hist(comparison_vectors_demo[,3], xlab = "Last Name Score", main = "", cex.lab=1.4, cex.axis=1.4)
hist(comparison_vectors_demo[,4], xlab = "First Name Score", main = "", cex.lab=1.4, cex.axis=1.4)
hist(comparison_vectors_demo[,5], xlab = "Middle Name Score", main = "", cex.lab=1.4, cex.axis=1.4)
par(mfrow=c(1,1))
...
links_demo <- comparison_vectors_demo %>% filter(LastName >= threshold & FirstName >= threshold
                                                  & MiddleName >= threshold)
# nrow(links_demo) - 289

# 5. Obtain linked data set
### obtain individual data frames with matched records
ida_demo <- sub_a[links_demo$id1,] %>% mutate("id.a" = links_demo$id1) %>% 
  select(id.a, LastName, FirstName, MiddleName, BirthYear, FirstIssueDate)
idb_demo <- sub_b[links_demo$id2,] %>% mutate("id.b" = links_demo$id2) %>% 
...
### linked data set
lds_demo <- cbind(ida_demo, idb_demo)
